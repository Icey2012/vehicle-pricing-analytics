...
| 阶段 | 关键决策/事件 | 判断依据 |
| :--- | :--- | :--- |
| 数据获取 | 爬虫爬取数据失败，遂打算利用手头243条/80+品牌的现成车型数据，考虑到数据太少，对于能不能做定价分析心中存疑 | 参考AI建议"随机森林对200条不敏感"，虽然直觉认为样本量不够，但未继续验证就启动了项目 |
| 缺失值处理 | 数据缺失较多，删除缺失值会导致样本更稀少，所以选择填充缺失值 | 观察到缺失严重的特征列大多有相关性高的特征列可以互相参照，如轴距和车身长度数据交错缺失，根据汽车设计知识（轴距与车长高度线性相关），分别用线性回归预测填充 |
| 数据清洗 | 原数据格式极其混乱，尤其是特征值格式中包括大量字符串和数字的混合形式，以及特征值不是明确数字而是数值范围的情况 | 拆分字符串和数值，用均值或中位数代替格式为数值范围的特征值 |
| 建模诊断 | 回归 R² 异常低，结合数据探索报告思量，原因大概不在于模型选择或参数设置 | 考虑到数据稀少且分布不均衡，检查训练集和测试集的价格分布是否一致，发现两者均值相差6万元→用 StratifiedShuffleSplit 分层采样校准。观察分类模型后发现价格分段与车型 segment 高度重叠——模型学的不是定价逻辑而是尺寸规律，继续优化特征工程或模型策略均无意义，不如系统性复盘，加强对ML项目正式启动前的可行性判断 ｜
| 系统复盘 | 小样本 ML项目到底能不能做？ | 总结了三类"先天友好"场景（强物理规律、预训练迁移、显式规则），输出可复用的可行性判断框架。本项目若想成功，至少需要上千条有效数据，各品牌数据分布应相对均衡，且需要同品牌同级别仅单一配置差异的成对样本 |
